...
public class GuiRender {
    ...
    public GuiRender(Window window) {
        ...
        shaderModuleDataList.add(new ShaderProgram.ShaderModuleData("/shaders/gui.vert", GL_VERTEX_SHADER));
        shaderModuleDataList.add(new ShaderProgram.ShaderModuleData("/shaders/gui.frag", GL_FRAGMENT_SHADER));
    ...
    private void createUIResources(Window window) {
        ...
        glfwGetFramebufferSize(window.getWindowHandle(), fbWidth, fbHeight);

        imGuiIO.setDisplaySize(fbWidth[0], fbHeight[0]);
        // --- NEW MAC FIX END ---

        float[] scaleX = new float[1];
        float[] scaleY = new float[1];
        glfwGetWindowContentScale(window.getWindowHandle(), scaleX, scaleY);
        imGuiIO.setFontGlobalScale(scaleX[0]);
