...
public class ModelLoader {
    ...
    public static Model loadModel(String modelId, String modelPath, TextureCache textureCache, int flags) {
        ...
        if (numAnimations > 0) {
            com.engine.scene.Node rootNode = buildNodesTree(aiScene.mRootNode(), null);
    ...
    private static List<Model.Animation> processAnimations(AIScene aiScene, List<Bone> boneList,
            com.engine.scene.Node rootNode, Matrix4f globalInverseTransformation) {
        List<Model.Animation> animations = new ArrayList<>();
        ...
        for (int i = 0; i < numAnimations; i++) {
            ...
            Model.Animation animation = new Model.Animation(aiAnimation.mName().dataString(), aiAnimation.mDuration(),
                    frames);
    ...
    private static void buildFrameMatrices(AIAnimation aiAnimation, List<Bone> boneList,
            ...
            int frame, com.engine.scene.Node node, Matrix4f parentTransformation, Matrix4f globalInverseTransform) {
        ...
        for (com.engine.scene.Node childNode : node.getChildren()) {
    ...
    private static Matrix4f buildNodeTransformationMatrix(AINodeAnim aiNodeAnim, int frame) {
        ...
        if (numRotations > 0) {
            AIQuatKey quatKey = rotationKeys.get(Math.min(numPositions - 1, frame));
    ...
    private static AINodeAnim findAIAnimNode(AIAnimation aiAnimation, String nodeName) {
        ...
        PointerBuffer aiChannnels = aiAnimation.mChannels();

        for (int i = 0; i < numNodeAnims; i++) {
            AINodeAnim aiNodeAnim = AINodeAnim.create(aiChannnels.get(i));
    ...
    private static com.engine.scene.Node buildNodesTree(AINode aiNode, Node parentNode) {
        String nodeName = aiNode.mName().dataString();
        com.engine.scene.Node node = new Node(nodeName, parentNode, toMatrix(aiNode.mTransformation()));
        ...
        for (int i = 0; i < numChildren; i++) {
            ...
            com.engine.scene.Node childNode = buildNodesTree(aiChildNode, node);
